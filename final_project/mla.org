...
| Model Description      | Training Perplexity | KV Cache Parameters / Token / Layer |
|------------------------+---------------------+-------------------------------------|
| MHA 35M                |               59.25 |                                8192 |
...
| MLA 279M               |               45.45 |                                6144 |
| MLA 292M no q compress |               43.34 |                                3072 |
...
Worth noting is the model's failure to outperform MQA at ~300M parameters. This seems likely related to q vector compression, which by default in the modeling source code uses a lora rank of half the size of the model (i.e. equivalent to not using compression on Q at all). I am going to run an experiment with better values for this, and see if that changes the results at all.
