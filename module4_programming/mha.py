...
class CustomMHA(torch.nn.Module):
    ...
    def split_into_heads(self, m):
        B, S, D = m.size()
        return m.view(B, S, self.h, self.dh).transpose(1, 2)

    def merge_heads(self, m):
        B, H, S, DH = m.size()
        return m.transpose(1, 2).contiguous().view(B, S, self.d)
    ...
    def forward(self, x):
        ...
        head_outs = self.attn(q_heads, k_heads, v_heads)

        # concat together and project with W_o
        pre_o = self.merge_heads(head_outs)
